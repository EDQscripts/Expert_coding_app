...
##        self.trackerName = ""
    ...
    def extractData(self):
        ...
        self.fileWriter.writerow(['Coded by', 'File', 'Eye', 'Target#', 'Onset (sec)',
                                  'Acceptable start (sec)', 'start quality', 'start delay', 'Acceptable end (sec)', 'end quality', 'end delay',
                                  'Window start (sec)', 'Window end (sec)', 'window quality', 'window delay', 'window duration',
            ...
            def mouse_move(self, event):
                ...
                if self.clicks == 0:
                    ...
                    if not (self.clicks == 2 and self.aE - self.aS < self.timeWindow):
                        self.ly_wS.set_xdata( min([max([x,self.aS]), self.aE]) )
                        self.ly_wS_2.set_xdata( min([max([x,self.aS]), self.aE]) )
                        self.ly_wE.set_xdata( min([max([x,self.aS]), self.aE]) + self.timeWindow )
                        self.ly_wE_2.set_xdata( min([max([x,self.aS]), self.aE]) + self.timeWindow )
...
##            endTime   = data['time'][first+1] + self.targetDuration# + self.timeAfterTarget  # go to end of target + 000 msec

            #find ending position of endTime
            last = start + 1 #len(data['time'])
            # Step through from first until you get to there
            while last < len(data['ROW_INDEX']) and data['ROW_INDEX'][start] == data['ROW_INDEX'][last]: # and data['time'][last] <= endTime:
                last += 1

            #print(RowIndex, first, last, data['ROW_INDEX'][first], data['ROW_INDEX'][first+1], data['ROW_INDEX'][last-1], data['ROW_INDEX'][last])
        ...
        for figure in figs:
            setDataAndLimits(figure, self.data, self.targetList[figure[4]])
...
##                            end = self.data['# count'].index( int(round(cursorPosition + self.fixationWindowSec*self.Hz)) )
                            ...
                            targetX = self.data['posx'][beg]
                            targetY = self.data['posy'][beg]
                            #print(targetX,targetY)
                            
                            dists = [ dist(targetX,targetY, xdats2[i],ydats2[i]) for i in range(len(xdats2)) ]
                            ...
                            self.fileWriter.writerow([self.coder, # Coder ID ('anon' default)
                                                      ...
                                                      "{:.3f}".format( min2(xdats) ),
                                                      "{:.3f}".format( max2(xdats) ),
